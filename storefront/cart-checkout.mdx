...
The system utilizes intelligent exemption rules implemented in [`app/Services/TwoFactorAuthService.php`](/app/Services/TwoFactorAuthService.php):
...
- **Component**: [`app/Livewire/TwoFactorVerify.php`](/app/Livewire/TwoFactorVerify.php)
- **View**: [`resources/views/livewire/two-factor-verify.blade.php`](/resources/views/livewire/two-factor-verify.blade.php)
- **UI Elements**:
...
### Site Labels (`site_labels` table)

| Label Key | Section | English Default | Spanish Translation (`es`) | French Translation (`fr`) |
|---|---|---|---|---|
| `checkout.field_password_required` | `checkout` | Create an Account Password | Crear una contraseña de cuenta | Créer un mot de passe de compte |
| `checkout.field_password_required_message` | `checkout` | An account password is required to complete your order and track your purchases. | Se requiere una contraseña de cuenta para completar su pedido y realizar el seguimiento de sus compras. | Un mot de passe de compte est requis pour finaliser votre commande et suivre vos achats. |
| `two_factor.checkout_heading` | `checkout` | Verify Your Purchase | Verifique su compra | Vérifiez votre achat |
| `two_factor.login_heading` | `auth` | Two-Factor Verification | Verificación de dos factores | Vérification à deux facteurs |
| `two_factor.subheading` | `auth` | We sent a 6-digit security verification code to | Enviamos un código de verificación de seguridad de 6 dígitos a | Nous avons envoyé un code de vérification de sécurité à 6 chiffres à |
| `two_factor.code_label` | `auth` | Enter 6-Digit Code | Ingrese el código de 6 dígitos | Entrez le code à 6 chiffres |
| `two_factor.continue_checkout_btn` | `checkout` | Verify & Continue to Payment → | Verificar y continuar al pago → | Vérifier et continuer vers le paiement → |
| `two_factor.verify_login_btn` | `auth` | Verify & Sign In → | Verificar e iniciar sesión → | Vérifier et se connecter → |
| `two_factor.verifying` | `auth` | Verifying code… | Verificando código… | Vérification du code… |
| `two_factor.did_not_receive` | `auth` | Didn't receive the code? | ¿No recibió el código? | Vous n'avez pas reçu le code ? |
| `two_factor.resend_in` | `auth` | Resend code in | Reenviar código en | Renvoyer le code dans |
| `two_factor.resend_button` | `auth` | Click here to resend code | Haga clic aquí para reenviar el código | Cliquez ici pour renvoyer le code |
| `two_factor.sending_code` | `auth` | Sending new code… | Enviando nuevo código… | Envoi du nouveau code… |
| `two_factor.return_to_checkout` | `checkout` | Return to Checkout | Volver a finalizar compra | Retour à la commande |
| `two_factor.return_to_login` | `auth` | Back to Sign In | Volver a iniciar sesión | Retour à la connexion |
...
### Models & Services
- [`app/Services/TwoFactorAuthService.php`](/app/Services/TwoFactorAuthService.php) — Core 2FA security service, code generator, session manager, and exemption evaluator.
- [`app/Models/User.php`](/app/Models/User.php) — User model with `last_login_at` timestamp tracking and guest sentinel helper `isGuest()`.
- [`app/Services/EmailTemplateService.php`](/app/Services/EmailTemplateService.php) — Dispatches multilingual 2FA email notifications.

### Controllers & Livewire Components
- [`app/Livewire/AdminSettings.php`](/app/Livewire/AdminSettings.php) — Admin settings controller managing `disable_guest_checkout`, `enable_checkout_2fa`, and `enable_login_2fa`.
- [`app/Livewire/TwoFactorVerify.php`](/app/Livewire/TwoFactorVerify.php) — 2FA verification landing form controller.
- [`app/Livewire/Checkout.php`](/app/Livewire/Checkout.php) — Checkout details form with dynamic password validation and 2FA challenge initiation.
- [`app/Livewire/OrderReview.php`](/app/Livewire/OrderReview.php) — Final order review guarded against unverified 2FA challenges.
- [`app/Livewire/Forms/LoginForm.php`](/app/Livewire/Forms/LoginForm.php) — Authentication form intercepting credentials for 2FA.
- [`app/Http/Controllers/Auth/SocialAuthController.php`](/app/Http/Controllers/Auth/SocialAuthController.php) — OAuth social login controller with role-based post-login redirection.

### Views & Templates
- [`resources/views/livewire/admin-settings.blade.php`](/resources/views/livewire/admin-settings.blade.php) — Admin settings UI toggles for 2FA and Guest Checkout.
- [`resources/views/livewire/two-factor-verify.blade.php`](/resources/views/livewire/two-factor-verify.blade.php) — 2FA verification form Blade view with Alpine.js countdown timer.
- [`resources/views/livewire/checkout.blade.php`](/resources/views/livewire/checkout.blade.php) — Checkout details Blade view with dynamic password fields and labels.
- [`resources/views/livewire/pages/auth/login.blade.php`](/resources/views/livewire/pages/auth/login.blade.php) — Sign-in form Blade view.
