...
### Incomplete Social Profile Gating

Social accounts that register with **empty or incomplete profile data** (e.g., no name, no email) are gated at checkout. These users are redirected to a profile completion screen and required to provide full shipping details before an order can be placed.

This ensures that guest-style OAuth signups do not result in unfulfillable orders due to missing customer information.
